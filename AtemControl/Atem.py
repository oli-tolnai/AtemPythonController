...
def consoleText():
    ...
    if lock == "Locked":
        ...
        print(sColor,end="")
    print(f" - SÚGÓ: {sugo}")
    print(f" - {pc_videsgra}: {pc_videsgraFADE}")
...
def on_press(key):
    ...
    global PVW
    global PGM
    global pc_videsgraFADE
    global pc_videsgra
    global pc_videsgraNumber
    ...
    if focus_window_pid == current_process_pid:
        ...
        #PC_VID és PC_GRA kiválasztása:
        if k == "v" and pressed == False and lock == "Unlocked" and pc_videsgra == "PC_GRA":
            pc_videsgra = "PC_VID"
            pc_videsgraNumber = 11
            setPGMandPVWtoCurrent()
            consoleText()
            pressed = True
        if k == "g" and pressed == False and lock == "Unlocked" and pc_videsgra == "PC_VID":
            pc_videsgra = "PC_GRA"
            pc_videsgraNumber = 12
            setPGMandPVWtoCurrent()
            consoleText()
            pressed = True
        ...
        if key == Key.page_up and pressed == False and lock == "Unlocked":
            if pc_videsgraFADE == "OFF":
                pc_videsgraFADE = "ON"
                atem4K.setPreviewInputVideoSource(1, pc_videsgraNumber) #ME2 Stream #2nd number is the number of PC_VID input
                atem4K.setPreviewInputVideoSource(2, pc_videsgraNumber) #ME3 Közönség
                atem4K.execAutoME(1)
                atem4K.execAutoME(2)
            else:
                pc_videsgraFADE = "OFF"
